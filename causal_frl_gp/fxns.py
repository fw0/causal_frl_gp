...
def lam_to_cov_fs(K, lam):
#    return np.linalg.inv(np.linalg.inv(K) + np.diag(lam**1))
    return np.linalg.inv(np.linalg.inv(K) + np.diag(lam**2))

def v_bar_and_lam_bar(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K, cov_fs=None):
...
#    v_bar, lam_bar = v_bar_and_lam_bar(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K, cov_fs)
    v_bar, lam_bar = v_bar_and_lam_bar(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam**2, ls, ys, K, cov_fs)
...
def F_fs(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K):
...
#        + np.sum(K_inv * cov_fs) # fix, possibly for efficiency
        + (0.5 * np.sum(K_inv * cov_fs))
        + (0.5 * np.dot(v.T, np.dot(K, v)))
        - (0.5 * np.log(np.linalg.det(cov_fs))) # fix: should compute determinant using cholesky factor
...
def variational_fs_posterior(alphas_prec_ys_0, betas_prec_ys_0, mu_c_0, prec_c_0, alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K, debug=False):
    ...
    def scalar_F_and_dF_ds(s):
        v = s[0:N]
        lam = s[N:]
        val = F_fs(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K) # fix: redundant calculations, like calculating cov_fs, expected values of stuff
        dF_dv, dF_dlam = dF_fs_dv_and_dlam(alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K)
...
@basic.do_cprofile('time')
def variational_posterior(num_tries, max_iter, alphas_prec_ys_0, betas_prec_ys_0, mu_c_0, prec_c_0, ls, ys, K, debug=False, eps=0.001):
    ...
    for i in xrange(num_tries):
        ...
        for j in xrange(max_iter):
            ...
            v, lam = variational_fs_posterior(alphas_prec_ys_0, betas_prec_ys_0, mu_c_0, prec_c_0, alphas_prec_ys, betas_prec_ys, mu_c, prec_c, v, lam, ls, ys, K, debug=debug)
...
def optimize_theta(theta_method, theta_num_tries, theta_max_iter, variational_num_tries, variational_max_iter, alphas_prec_ys_0, betas_prec_ys_0, mu_c_0, prec_c_0, ls, ys, xs, k, theta_init_f, debug=False, theta_grid=None):
    # gradient free hyperparam optimization.  returns best theta and associated evidence

    def objective(theta):
        K = k(theta, xs, xs)
        posterior, evidence = variational_posterior(variational_num_tries, variational_max_iter, alphas_prec_ys_0, betas_prec_ys_0, mu_c_0, prec_c_0, ls, ys, K, debug=False)
        return evidence
...
def rule_search(search_num_tries, search_num_iter, theta_method, theta_num_tries, theta_max_iter, variational_num_tries, variational_max_iter, alphas_prec_ys_0_f, betas_prec_ys_0_f, mu_c_0_f, prec_c_0_f, all_cs, ts, ys, xs, k, theta_init_f, rule_idxs_init_f, debug=False, theta_grid=None, min_support=2):
    # moves are insert, remove, permute, replace

    def objective(rule_idxs):
        cs = np.concatenate((all_cs[:,rule_idxs], np.ones((len(ys),1))), axis=1)
        zs = cs_to_zs(cs)
        ls = zs_to_ls(zs) * ts[:,np.newaxis]
        theta, evidence = optimize_theta(theta_method, theta_num_tries, theta_max_iter, variational_num_tries, variational_max_iter, alphas_prec_ys_0_f(rule_idxs), betas_prec_ys_0_f(rule_idxs), mu_c_0_f(rule_idxs), prec_c_0_f(rule_idxs), ls, ys, xs, k, theta_init_f, debug=debug, theta_grid=theta_grid)
        return evidence

    def pre_reject(rule_idxs):
        cs = np.concatenate((all_cs[:,rule_idxs], np.ones((len(ys),1))), axis=1)
        zs = cs_to_zs(cs)
        treated_ls = np.sum(zs * ts[:,np.newaxis], axis=0)
        control_ls = np.sum(zs * ((ts==0).astype(int))[:,np.newaxis], axis=0)
#        pdb.set_trace()
